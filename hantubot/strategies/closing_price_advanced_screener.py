...
class ClosingPriceAdvancedScreener(BaseStrategy):
    ...
    def __init__(self, strategy_id: str, config: Dict[str, Any], broker: Broker, clock: MarketClock, notifier: Notifier):
        ...
        self.webhook_time = dt.time(15, 3)  # 웹훅 알림 시간 15:03
        self.buy_start_time = dt.time(15, 15)  # 매수 시작 시간 15:15
        self.buy_end_time = dt.time(15, 19)  # 매수 종료 시간 15:19
        ...
        self.has_webhook_sent_today = False  # 15:03 웹훅 발송 여부
        self.has_bought_today = False  # 15:15-19 매수 실행 여부
        self.top_stocks_today = []  # 당일 스크리닝 결과 저장
    ...
    async def _perform_screening(self, data_payload: Dict[str, Any], top_volume_stocks: List[Dict]) -> List[Dict[str, Any]]:
        ...
        Returns:
            screened_stocks: 점수 순 정렬된 종목 리스트
        """
        screened_stocks = []
        ...
        for stock_data in top_volume_stocks:
            ...
            try:
                ...
                if pd.isna(sma20) or sma20 == 0:
                    ...
                    score_sma = min(100, 50 + gap_from_sma * 5) if current_price > sma20 else max(0, 50 + gap_from_sma * 5)

                total_score = (score_cci * 0.25) + (score_volume * 0.25) + (score_adx * 0.15) + (candle_score * 0.25) + (score_sma * 0.10)
    ...
    async def generate_signal(self, data_payload: Dict[str, Any], portfolio: Portfolio) -> List[Dict[str, Any]]:
        ...
        # ========================================
        if self.webhook_time <= now.time() < self.buy_start_time and not self.has_webhook_sent_today:
            ...
            try:
                ...
                # TOP3 추출 및 저장
                self.top_stocks_today = screened_stocks[:self.top_n_screen]
            ...
            return signals  # 15:03에는 매수 신호 생성 안함!
        
        # ========================================
        # 15:15-15:19: 저장된 1위 종목 매수
        # ========================================
        if self.buy_start_time <= now.time() <= self.buy_end_time and not self.has_bought_today:
